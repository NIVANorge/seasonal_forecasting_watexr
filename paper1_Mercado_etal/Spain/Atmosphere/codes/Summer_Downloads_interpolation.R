...
loginUDG("WATExR", "1234567890")
summer <- lapply(1:length(variables), function(x) loadSeasonalForecast(dataset, dictionary = dictionary,
...
names(summer) <- c(variables[1:10], "tasmax", "tasmin")
summer$tas <- udConvertGrid(summer$tas, new.units = "celsius")
summer$tasmin <- udConvertGrid(summer$tasmin, new.units = "celsius")
summer$tasmax <- udConvertGrid(summer$tasmax, new.units = "celsius")
summer$psl <- udConvertGrid(summer$psl, new.units = "millibars")
summer$petH<-petGrid(tasmin=summer$tasmin, tasmax=summer$tasmax, method = "hargreaves-samani")
directory <- "/home/ry4902/Documents/Workflow/Atmosphere/SEAS5/hindcast/"
save(summer, file=paste(directory, "summer.RData", sep=""))

# Bilinear interpolation of the data to the location of the lake
summer.interp <- lapply(summer, function(x) interpGrid(x, new.coordinates = lake, 
                                                       method = "bilinear", 
                                                       bilin.method = "akima"))

save(summer.interp, file=paste(directory, "Interpolation/summer.RData", sep=""))
