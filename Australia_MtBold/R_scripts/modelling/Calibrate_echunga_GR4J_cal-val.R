setwd("C:\\Users\\mooret\\OneDrive - Dundalk Institute of Technology\\WateXr\\Mt_Bold_Data/Echunga/")
...
inp <- read.csv("C:\\Users\\mooret\\OneDrive - Dundalk Institute of Technology\\WateXr\\Mt_Bold_Data\\Echunga/echunga_flow_cumecs_2003_2013.csv", stringsAsFactors = F)
...
write.csv(param_out, 'echunga_obs_gr4j_calib_param_cal-period.csv', row.names = F, quote = F)
#Param <- c(474.13,-2.65,12.69,0.5) #Param from excel

OutputsModel <- RunModel_GR4J(InputsModel = inputs, RunOptions = opts, Param = Param)
str(OutputsModel)

png('Echunga_GR4J_obs_Calib_results_all.png', height = 900, width = 1600, res =120)
...
run_gr4j(time = met$date, pet = met$et_morton_potential, pre = met$daily_rain, airt = met$mean_temp, warmup_unit = 'day', warmup_n = wup, param = Param, catch_size = catch.area, out_file = 'echunga_obs_GR4J_flow_1980_2018.csv', calc_T = F, vector = F, model = 'GLM')
...
write.csv(Param, 'echunga_obs_gr4j_calib_param.csv', row.names = F, quote = F)
######
...
png('Echunga_GR4J_ERA5_Calib_results_all.png', height = 900, width = 1600, res =120)
plot(OutputsModel, Qobs = inp.cal$flow.mm.day)
...
write.csv(Param2, 'echunga_ERA5_gr4j_calib_param.csv', row.names = F, quote = F)

# GR5J ----
inputs <- CreateInputsModel(RunModel_GR5J, DatesR = met$X.DateTime, Precip = met$tp_mm, PotEvap = met$mper_mm.d...1)
Ind_Run <- which(met[,1] >= inp.cal[1,1] & met[,1] <= inp.cal[nrow(inp.cal),1])
warm_up <- 1:(Ind_Run[1]-1)
opts <- CreateRunOptions(RunModel_GR5J, InputsModel = inputs,IndPeriod_Run = Ind_Run, IndPeriod_WarmUp = warm_up)
...
OutputsCalib <- Calibration_Michel(InputsModel = inputs, RunOptions = opts, InputsCrit = err_crit, CalibOptions = CalibOptions, FUN_MOD = RunModel_GR5J)

Param <- OutputsCalib$ParamFinalR
Param 

OutputsModel <- RunModel_GR5J(InputsModel = inputs, RunOptions = opts, Param = Param)
str(OutputsModel)

nse <- NSE(OutputsModel$Qsim, inp.cal$flow.mm.day)
kge <- KGE(OutputsModel$Qsim, inp.cal$flow.mm.day)

png('Echunga_GR5J_ERA5_Calib_results_all.png', height = 900, width = 1600, res =120)
plot(OutputsModel, Qobs = inp.cal$flow.mm.day)
mtext(paste0("KGE = ", round(kge,2), "; NSE = ", round(nse, 2)), side=3, cex = 2)
dev.off()


Param2 <- data.frame(prod.stor.cap_mm = Param[1], inter.exch.coeff_mm.d = Param[2], rout.stor.cap_mm = Param[3], unit.hyd.time.cons_d = Param[4], inter.exch.thresh = Param[5], NSE = nse, KGE = kge)
write.csv(Param2, 'echunga_ERA5_gr5j_calib_param.csv', row.names = F, quote = F)

# GR6J ----
inputs <- CreateInputsModel(RunModel_GR6J, DatesR = met$X.DateTime, Precip = met$tp_mm, PotEvap = met$mper_mm.d...1)
Ind_Run <- which(met[,1] >= inp.cal[1,1] & met[,1] <= inp.cal[nrow(inp.cal),1])
warm_up <- 1:(Ind_Run[1]-1)
...
png('Echunga_GR6J_ERA5_Calib_results_all.png', height = 900, width = 1600, res =120)
plot(OutputsModel, Qobs = inp.cal$flow.mm.day)
mtext(paste0("KGE = ", round(kge,2), "; NSE = ", round(nse, 2)), side=3, cex = 2)
dev.off()

Param2 <- data.frame(prod.stor.cap_mm = Param[1], inter.exch.coeff_mm.d = Param[2], rout.stor.cap_mm = Param[3], unit.hyd.time.cons_d = Param[4], inter.exch.thresh = Param[5], coeff.emp.exp.stor = Param[6], NSE = nse, KGE = kge)
write.csv(Param2, 'echunga_ERA5_gr6j_calib_param.csv', row.names = F, quote = F)

# Create inflow file ----

wup = 0

Param <- read.csv('echunga_ERA5_gr4j_calib_param.csv')
Param <- unlist(Param)[1:4]

run_gr4j(time = met$X.DateTime, pet = met$mper_mm.d...1, pre = met$tp_mm, airt = met$t2m_celsius, warmup_unit = 'day', warmup_n = wup, param = Param, catch_size = catch.area, out_file = 'echunga_ERA5_GR4J_flow_1980_2019_noTemp.csv', calc_T = F, vector = F, model = 'GLM')
run_gr4j(time = met$X.DateTime, pet = met$mper_mm.d...1, pre = met$tp_mm, airt = met$t2m_celsius, warmup_unit = 'day', warmup_n = wup, param = Param, catch_size = catch.area, out_file = 'echunga_ERA5_GR4J_flow_1980_2019_wTemp.csv', calc_T = T, vector = F, model = 'GLM')
