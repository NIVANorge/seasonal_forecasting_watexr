...
dir.data <- 'C:/Data/GitHub/WATExR/Norway_Morsa/Data/Meteorological/' 
dir.Rdata <- 'C:/Data/GitHub/WATExR/Norway_Morsa/Data/Meteorological/RData/'
  
# Define the geographical domain to be loaded
latLim <- c(59.31, 59.90) # Morsa catchment
lonLim <- c(10.63, 11.25) # Morsa catchment
# Alternatively...
...
years <- 1981:2000 #This will be 1981-2010 once it's working
...
lakename <- "Morsa" # Just for setting filename in export
...
# Type str(variable name) to get info about the variable (though this isn't working, so may have remembered wrong..)
...
#data <- c(data.interp, "cc" = list(clt)) # Put all variables together

# Reassign data for use below
data = data.interp
...
# Create directory (if not already done) and save file
startTime <- format(as.POSIXlt(yymmdd[1]), format = "%Y%m%d")
endTime <- format(tail(as.POSIXlt(yymmdd), n = 1), format = "%Y%m%d")
dirName <- paste0(dir.data, lake_id, "_", institution, "_", ClimateModelName, "_", ExperimentName, "_", member, "_", freq, "_", startTime, "-", endTime, "/", sep = "", collapse = NULL)
